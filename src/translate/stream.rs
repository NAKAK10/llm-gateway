...
/// The last usage numbers seen upstream, in `openai-chat` names.
#[derive(Default)]
struct ChatUsage {
    prompt: u64,
    completion: u64,
...
impl ChatToAnthropic {
    ...
    fn record_usage(&mut self, usage: &Value) {
        ...
        self.usage.prompt = field("prompt_tokens");
        self.usage.completion = field("completion_tokens");
        self.usage.cached = usage
    ...
    fn handle_text(&mut self, delta: &Value, out: &mut Vec<u8>) {
        let text = match delta.get("content") {
            ...
            // Defensive: a few proxies mimic the Responses shape and send
            // content parts even in a chat stream.
    ...
    fn handle_tool_calls(&mut self, delta: &Value, out: &mut Vec<u8>) {
        ...
        for call in calls {
            ...
            // Forwarded verbatim: a fragment is not valid JSON on its own, and
            // the client reassembles the string before parsing it.
            let fragment = match function.and_then(|f| f.get("arguments")) {
                Some(Value::String(s)) if !s.is_empty() => Some(s.clone()),
                // Some servers send the whole argument object instead of a
                // string; serialise it once and emit it as a single fragment.
...
/// One Anthropic SSE frame. Both the `event:` name and the `type` inside the
/// payload are sent, because clients dispatch on the former and validate the
/// latter.
