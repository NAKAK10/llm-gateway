...
pub fn chat_error_to_anthropic(body: &Value, status: u16) -> Value {
    ...
    let message = match error {
        // The common `openai-chat` shape: `{"error": {"message": "…", …}}`.
        Some(Value::Object(_)) => error
            .and_then(|e| e.get("message"))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string()),
        // Ollama's shape: `{"error": "plain string"}`.
        Some(Value::String(s)) if !s.is_empty() => Some(s.clone()),
        _ => None,
    };
    let message = message.unwrap_or_else(|| format!("upstream returned HTTP {status}: {body}"));
