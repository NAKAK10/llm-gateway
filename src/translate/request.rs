...
//!
//! The two wire formats disagree on almost every interesting point: where the
//! system prompt lives, whether tool call arguments are a string or an
//! object, which message role carries a tool's result, and what a "list of
//! content blocks" even means. This module builds a **fresh** `openai-chat`
//! object field by field rather than patching the Anthropic one in place,
//! because passing an unrecognized Anthropic key through (`cache_control`,
//! `thinking`, `top_k`, …) makes strict `openai-chat` servers answer with a
//! `400`, which is worse than silently dropping something the target
