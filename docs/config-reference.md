...
| `transport` | `"http"` | `"http"` \| `"claude-cli"`. `claude-cli` runs the local `claude` binary instead of making a request, which is how a Claude subscription can serve gateway traffic — `baseUrl` and `apiKey` are then unused (the CLI holds its own login) and `api` must be `anthropic-messages`. See the README's "Subscription-backed providers". |
