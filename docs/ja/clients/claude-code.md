...
Claude Code のハーネス自身のテキストと紛らわしくマッチしうる description を
書くなら知っておく価値があります: Claude Code はすべてのセッションの最初の
user message に `<system-reminder>` ブロック(プロジェクトの `CLAUDE.md` など)
を注入しますが、そのブロックは比較が行われる前に分類の入力から除去されます
— そのため、ウォークバックが最初のメッセージまで遡ったからといって、
その定型文の力で description が勝ち続けることはありません。
