...
- **route の `description` は、もはや飾りではない。** 受信したすべての
  リクエストは、ワイルドカードではない各 route の `description` と埋め込みで
  比較される。似たり寄ったりの説明を 5 個並べれば、似たり寄ったりのスコアが
  5 個返る。プロバイダー名の言い換えではなく、役割の違いを書くこと。
- **分類閾値は `0.45` 固定。** `src/semantic/index.rs` の
  `CLASSIFICATION_THRESHOLD` にあり、route ごとのつまみも
  `routes[].semantic` ブロックも、もう存在しない。
- **`default` はフォールバックであると同時に候補でもある。** 他が閾値を
  超えなかったときの受け皿だが、自分の `description` が最も合っていれば
  普通に勝つ。空のバケツではなく、実在の route として扱うこと。
- **`init` にはネットワークと数百 MB の空きが要る。** static な埋め込み
  モデルを `config.json` より先に無条件でダウンロードする。避ける方法は
  default features を外したビルドだけで、その場合は全リクエストが `default`
  に流れる。
...
## Agent CLI トランスポート(`claude-cli`、`codex-cli`)
...
- **`codex-cli` も `OPENAI_API_KEY` を取り除く。** これが設定されたままだと、
  ユーザーがサブスクリプションで実行したいことに対して `codex` が API
  アカウントに課金してしまう — 目的の正反対であり、請求書を見るまで
  気付かない。
- **Codex のイベントはトークン単位ではなくアイテム単位。** `agent_message`
  は完成した状態で届くので、逐次ストリームするものが何もない —
  ゲートウェイは整形された `openai-chat` ストリームを合成し、それが一度に
  まとめて届く。これをテキストのチャンク分割で「直そう」としないこと —
  偽物のトークンストリームは、正直に遅れて届くものより悪い。
- **Codex はアップストリームのエラーを自分の `message` の中に JSON として
  入れ子にする。** 有用な文は内側のオブジェクトの `error.message` であり、
  封筒ごと転送するとユーザーにはエスケープされた引用符の壁が見えるだけで、
  「this model is not supported when using Codex with a ChatGPT account」
  のような本題が見えなくなる。
- **プランはどのモデルを試しても拒否してくることがある。** 検証済み:
  ある ChatGPT アカウントは `gpt-5`、`gpt-5.1`、`gpt-5-codex`、
  `gpt-5.1-codex`、さらにユーザー自身が設定した既定モデルまですべて
  拒否した。だから `codex-cli` の route は(`-m` フラグを付けずに)
  `default` を雛形にし、拒否はゲートウェイのエラーではなく CLI 自身の
  メッセージとしてそのまま表面化する。
