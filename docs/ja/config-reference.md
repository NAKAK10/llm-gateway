...
| `logging` | `false` | `serve` のコンソール(stderr)診断ログ — embedding モデルの準備状況や、フォールバック各試行の結果など。デフォルトでは無効(通常のゲートウェイプロセスは静かなままにするため)で、`true` にすると表示される。上記の `usage`/`debug` によるディスク上のログとは無関係で、`RUST_LOG` を明示的に指定した場合はそちらが優先される。 |
