...
到達できます。その方向は変換されるからです。これはターゲットごとに判定
されるので、1 つの route の中で `default` と `fallbacks` に異なる `api` の
プロバイダーを自由に混在させられます — クライアントのプロトコルから到達
できないターゲットは、リクエスト時に単にスキップされます。そのような
試行で何を諦めることになるかは
...
// Anthropic を話す route に、変換を挟まない同一プロトコルのフォールバック先を
// 与える。上の openai-chat id へのクロスプロトコルフォールバックの代替案。
...
一見、同じアカウントに 2 つの id があるのは冗長に見える。これは、1 つの
プロバイダーエントリが 1 つのアップストリームをちょうど 1 つのワイヤー
プロトコル(`api`)に結びつける、という制約が今も変わっていないためだ
(`src/config/mod.rs` 参照)。OpenRouter が `openai-chat` と
`anthropic-messages` のどちらを受け付けるかは認証情報ではなく POST 先の
パスで決まるので、同じキーを単純に 2 回登録している。route の
`fallbacks` はもう `default` と同じプロトコルである必要はない
(`docs/decisions.md` 参照)が、それでも `openrouter-anthropic` を登録する
価値はある: 上の `openai-chat` id へのクロスプロトコルフォールバックには
ない、同一プロトコル・変換なしのパススルーが手に入るからだ。`init` は
Claude Code のフォールバックとして OpenRouter を選んだ場合にのみ
`openrouter-anthropic` を追加する。
